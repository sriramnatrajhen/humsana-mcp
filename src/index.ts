...
interface Config {
  ...
  webhook_url?: string;
  write_warn_threshold: number;
  write_block_threshold: number;
}
...
async function verifyLicense(): Promise<LicenseResult> {
  // License gating removed. Humsana MCP is fully open source and 100% local:
  // live vs dry-run is controlled entirely by config.execution_mode.
  // No network calls, no license server, no phoning home.
...
function loadConfig(): Config {
  ...
  try {
    ...
    const LIST_KEYS = ["dangerous_commands", "deny_patterns", "allow_patterns"];
    ...
    for (const rawLine of lines) {
      ...
      // YAML list item under a list key, e.g.  - "rm -rf"
      if (currentList && trimmed.startsWith("-")) {
        const item = stripQuotes(trimmed.slice(1).trim());
        if (item) (config[currentList] as string[]).push(item);
        continue;
      }

      if (!trimmed.includes(":")) { currentList = null; continue; }

      const [key, ...valueParts] = trimmed.split(":");
      const value = valueParts.join(":").trim();

      // A list key with no inline value opens a list block (resets it to empty)
...
async function main() {
  ...
  console.error("🚀 Humsana MCP server v2.2.0 running (open source, 100% local)");
